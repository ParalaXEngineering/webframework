...
def displayer_test_app(test_app):
    """
    Extend the conftest test_app with displayer-specific routes.
    This avoids duplicating Flask app setup while adding test-specific endpoints.
    """
    app = test_app
    ...
    def url_for_with_params_filter(endpoint, params=None):
        ...
        Matches the filter defined in main.py for production use.
        """
...
def generate_index_page() -> None:
    ...
    # Go up two levels: test_displayer.py -> integration -> tests -> workspace_root
    workspace_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    template_path_framework = os.path.join(workspace_root, "templates")
    template_path_test = os.path.join(workspace_root, "tests", "templates")
...
def add_displayer_layout(disp: displayer.Displayer, layout_type: displayer.Layouts, table_mode: displayer.TableMode = None) -> None:
    ...
    For TABLE layouts, table_mode specifies which TableMode to use:
    - None (default): Simple table without DataTables
    - SIMPLE: Plain HTML table
    - INTERACTIVE: DataTables with manual rows
    - BULK_DATA: DataTables with pre-loaded JSON
    - SERVER_SIDE: DataTables with AJAX endpoint
    """
    disp.add_generic("AutoModule")
    ...
    if layout_type == displayer.Layouts.VERTICAL:
        ...
        if table_mode is None or table_mode == displayer.TableMode.SIMPLE:
            # Simple HTML table
            layout = displayer.DisplayerLayout(
                layout_type, 
                ["Name", "Status"],
                subtitle=f"Table ({table_mode.value if table_mode else 'simple'})"
            )
            disp.add_master_layout(layout)
            disp.add_display_item(displayer.DisplayerItemText("Row1"), 0)
            disp.add_display_item(displayer.DisplayerItemBadge("Active", displayer.BSstyle.SUCCESS), 1)
        
        elif table_mode == displayer.TableMode.INTERACTIVE:
            # DataTables with manual row population
            layout = displayer.DisplayerLayout(
                layout_type,
                ["Product", "Price"],
                subtitle="Table (interactive)",
                datatable_config={
                    "table_id": "test_interactive",
                    "mode": displayer.TableMode.INTERACTIVE
                }
            )
            disp.add_master_layout(layout)
            disp.add_display_item(displayer.DisplayerItemText("Laptop"), 0)
            disp.add_display_item(displayer.DisplayerItemText("$999"), 1)
            disp.add_display_item(displayer.DisplayerItemText("Mouse"), 0)
            disp.add_display_item(displayer.DisplayerItemText("$29"), 1)
        
        elif table_mode == displayer.TableMode.BULK_DATA:
            # DataTables with pre-loaded JSON
            bulk_data = [
            ...
            layout = displayer.DisplayerLayout(
                layout_type,
                ["Name", "Role"],
                subtitle="Table (bulk_data)",
                datatable_config={
                    "table_id": "test_bulk",
                    "mode": displayer.TableMode.BULK_DATA,
                    "data": bulk_data,
                    "columns": [{"data": "Name"}, {"data": "Role"}]
                }
            )
            disp.add_master_layout(layout)
        
        elif table_mode == displayer.TableMode.SERVER_SIDE:
            # DataTables with AJAX endpoint
            layout = displayer.DisplayerLayout(
                layout_type,
                ["ID", "Status"],
                subtitle="Table (server_side)",
                datatable_config={
                    "table_id": "test_server",
                    "mode": displayer.TableMode.SERVER_SIDE,
                    "api_endpoint": "test_disp.item_page",
...
def test_displayer_items(displayer_test_app: Flask, item_class: type) -> None:
    ...
    render_result = render_item_page(displayer_test_app, disp, target=target)
    ...
    if category == "input":
        ...
        if len(form_data) <= 1 and "csrf_token" in form_data:
            ...
            json_response = post_and_get_json(displayer_test_app, form_data)
...
def _get_layout_test_parameters():
    """
    Generate test parameters for all layouts including TABLE modes.
    Returns list of tuples: (layout_type, table_mode, test_id)
    """
    params = []
    for layout in displayer.Layouts:
        if layout == displayer.Layouts.TABLE:
            # Test all table modes
            for mode in displayer.TableMode:
                params.append((layout, mode, f"TABLE_{mode.value}"))
        else:
            # Non-table layouts don't have modes
            params.append((layout, None, layout.name))
    return params


@pytest.mark.parametrize("layout_type,table_mode,test_id", 
    _get_layout_test_parameters(),
    ids=lambda x: x[2] if isinstance(x, tuple) else str(x)
)
def test_displayer_layouts(displayer_test_app: Flask, layout_type: displayer.Layouts, 
                          table_mode: displayer.TableMode, test_id: str) -> None:
    """
    Test each DisplayerLayout type and all TABLE modes:
    1. Create layout with sample items
    2. Render page and save HTML
    3. Assert layout-specific HTML structure
    
    This single test now covers:
    - All basic layouts (VERTICAL, TABS, SPACER, etc.)
    - All TABLE modes (SIMPLE, INTERACTIVE, BULK_DATA, SERVER_SIDE)
    """
    ...
    # Build displayer with appropriate mode
    disp = displayer.Displayer()
    add_displayer_layout(disp, layout_type, table_mode)
    
    # Render page
    render_result = render_item_page(displayer_test_app, disp)
    
    # Save HTML for manual review with descriptive filename
    html_filename = f"layout_{test_id}.html"
    save_html(html_filename, render_result["html"])
    
    # Assert successful render
    assert render_result["status_code"] == 200, f"Rendering failed for {test_id}"
...
def test_displayer_basics(displayer_test_app: Flask) -> None:
    """
