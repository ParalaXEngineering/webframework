...
class LogEmitter:
    ...
    def _emit_loop(self):
         ...
         while self.running:
             try:
                 ...
                 with self._lock:
                     has_live_users = bool(self.live_mode_users)
                 
                 if has_live_users:
                     self._emit_log_content()
             except Exception as e:
                 self.logger.error("Error emitting log content: %s", e)
    ...
    def set_live_mode(self, username: str, enabled: bool):
         ...
         with self._lock:
             if enabled:
                 ...
                 self.logger.info("Live mode enabled for user: %s", username)
             else:
                 self.live_mode_users.discard(username)
                 self.logger.info("Live mode disabled for user: %s", username)
    ...
    def get_log_data(self, max_lines_per_file: int = 50) -> Dict:
         ...
         try:
             ...
             self.logger.error("Error getting log data: %s", e)
             return {'error': str(e)}
         ...
         try:
             ...
             for username in live_users:
                 try:
                     ...
                     self.logger.error("Error emitting log update to user %s: %s", username, e)
             
         except Exception as e:
             self.logger.error("Error in _emit_log_content: %s", e)
    ...
    def _read_and_parse_log_file(self, filepath: str, max_lines: Optional[int] = None) -> List[Dict]:
         ...
         try:
             ...
             self.logger.error("Error reading log file %s: %s", filepath, e)
             return []
    ...
    def _truncate_log_file(self, filepath: str):
         ...
         try:
             ...
             self.logger.error("Error truncating log file %s: %s", filepath, e)
    ...
    def _parse_log_line(self, line: str) -> Dict:
         ...
         match1 = re.match(PATTERN_STANDARD, line.strip())
         ...
         match2 = re.match(PATTERN_FALLBACK, line.strip())
