...
def config_edit():
    ...
    for item in session['config']:
        for subitem in session["config"][item]:
            if "type" in session["config"][item][subitem] and session["config"][item][subitem]["type"] == "serial_select":
    ...
    if "access" in session["config"]:
        ...
        session['config']["access"]["modules"]["value"]["Website engine update creation"] = ["admin"]
        session['config']["access"]["modules"]["value"]["Website engine update"] = ["admin", "user"]
    ...
    for group in config:
        disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [12], subtitle=config[group]["friendly"]))
        for item in config[group]:
            if "friendly" in config[group][item] and item != "friendly":
                if config[group][item]["type"] == "string":
                    ...
                    disp.add_display_item(displayer.DisplayerItemText(config[group][item]["friendly"]), 0)
                    disp.add_display_item(displayer.DisplayerItemInputText(group + "." + item, None, config[group][item]["value"]), 1)
                elif config[group][item]["type"] == "int":
                    disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [3, 9]))
                    disp.add_display_item(displayer.DisplayerItemText(config[group][item]["friendly"]), 0)
                    disp.add_display_item(displayer.DisplayerItemInputNumeric(group + "." + item, None, config[group][item]["value"]), 1)
                elif config[group][item]["type"] == "select":
                    disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [3, 9]))
                    disp.add_display_item(displayer.DisplayerItemText(config[group][item]["friendly"]), 0)
                    disp.add_display_item(displayer.DisplayerItemInputSelect(group + "." + item, None, config[group][item]["value"], config[group][item]["options"]), 1)
                elif config[group][item]["type"] == "serial_select":
                    disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [3, 9]))
                    disp.add_display_item(displayer.DisplayerItemText(config[group][item]["friendly"]), 0)
                    disp.add_display_item(displayer.DisplayerItemInputSelect(group + "." + item, None, config[group][item]["value"], serial), 1)
                elif config[group][item]["type"] == "cat_icon":
                    disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [3, 9]))
                    disp.add_display_item(displayer.DisplayerItemText(config[group][item]["friendly"]), 0)
                    disp.add_display_item(displayer.DisplayerItemInputTextIcon(group + "." + item, config[group][item]["value"]), 1)
                elif config[group][item]["type"] == "multistring":
                    disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [3, 3, 6]))
                    disp.add_display_item(displayer.DisplayerItemText(config[group][item]["friendly"]), 0)
                    for line in config[group][item]: 
                        if line != "type" and line != "friendly":
                            disp.add_display_item(displayer.DisplayerItemText(config[group][item][line]["friendly"]), 1)
                            disp.add_display_item(displayer.DisplayerItemInputText(group + "." + item + "." + line, None, config[group][item][line]["value"]), 2)
                            disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [3, 3, 6]))
                elif config[group][item]["type"] == "constrained_list":
                    disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [3, 9]))
                    disp.add_display_item(displayer.DisplayerItemText(config[group][item]["friendly"]), 0)
                    disp.add_display_item(displayer.DisplayerItemInputMultiSelect(group + "." + item, None, config[group][item]["value"], config[group][item]["options"]), 1)
                elif config[group][item]["type"] == "free_list":
                    disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [3, 9]))
                    disp.add_display_item(displayer.DisplayerItemText(config[group][item]["friendly"]), 0)
                    disp.add_display_item(displayer.DisplayerItemInputMultiText(group + "." + item, None, config[group][item]["value"]), 1)
                elif config[group][item]["type"] == "constrained_mapping":
                    disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [3, 9]))
                    disp.add_display_item(displayer.DisplayerItemText(config[group][item]["friendly"]), 0)
                    disp.add_display_item(displayer.DisplayerItemInputMapping(group + "." + item, None, config[group][item]["value"], config[group][item]["constrains"]), 1)
                elif config[group][item]["type"] == "mapping":
                    disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [3, 9]))
                    disp.add_display_item(displayer.DisplayerItemText(config[group][item]["friendly"]), 0)
                    disp.add_display_item(displayer.DisplayerItemInputMapping(group + "." + item, None, config[group][item]["value"]), 1)

    disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [12], alignment=[displayer.BSalign.R]))
    disp.add_display_item(displayer.DisplayerItemButton("update", "Update"), )     
    return render_template('base_content_new.j2', content=disp.display(), target="settings.config_apply")
    ...
    for group in data_json:
        for item in data_json[group]:
            if "_list" not in item and "mapleft" not in item and "mapright" not in item:
                if session["config"][group][item]["type"] == "string" or \
                        session["config"][group][item]["type"] == "int" or \
                        session["config"][group][item]["type"] == "select" or \
                        session["config"][group][item]["type"] == "serial_select":
                    session["config"][group][item]["value"] = data_json[group][item]
                elif session["config"][group][item]["type"] == "free_list" or \
                        session["config"][group][item]["type"] == "constrained_list" or \
                        session["config"][group][item]["type"] == "s":
