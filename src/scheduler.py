...
class Scheduler:
    ...
    def emit_status(self, category: str, string: str, status: int = 0, supplement: str = ""):
        ...
        :type string: str
        :param status: The status, a number between 0 and 100% that indicate the progress. At 100% the task is considered as successfull. The status 101% can be used to indicate an error, defaults to 0
    ...
    def emit_result(self, category: str, content):
        """Add a result information in the bottom of the "Action progress". The category is any category supported by bootstrap (success, danger, etc...)
        ...
        while(1):
            ...
            if len(filtered_status) > 0:
                for item in reversed(filtered_status):
                    ...
                    self.socket_obj.emit('action_status', {item[0]: [item[1], item[2], item[3]]})
            ...
            for name in threads_names:
                ...
                if current_thread:
                    thread_info.append({"name": name, "state": current_thread.m_running_state})
