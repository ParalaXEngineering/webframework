...
class SETUP_Packager(threaded_action.Threaded_action):
    ...
    def action(self):
        ...
        if self.m_action == "create_package":
            ...
            self.m_scheduler.emit_status(self.get_name(), "Creating archive, this might take a while", 103)
            today = datetime.datetime.now()
            try:
                shutil.make_archive(os.path.join("packages", today.strftime("%y%m%d_" + self.m_file)), "zip", os.path.join("ressources"))
                self.m_scheduler.emit_status(self.get_name(), "Creating archive, this might take a while", 100)
            except Exception as e:
                self.m_scheduler.emit_status(self.get_name(), "Error in creating the archive: " + str(e), 101)

            to_upload_files = utilities.util_dir_structure(os.path.join("packages"), inclusion=[".zip"])
            print(to_upload_files)
            reloader = utilities.util_view_reload_input_file_manager(self.m_default_name, "upload_package", [to_upload_files], ["Files to upload"], ["file-upload"], ["primary"], [False])
            print(reloader)
            ...
            if config["updates"]["source"]["value"] == "Folder":
                try:
                    os.mkdir(os.path.join(config["updates"]["folder"]["value"], "packages"))
                except FileExistsError as e:
                    pass

                try:
                    os.mkdir(os.path.join(config["updates"]["folder"]["value"], "packages", site_conf_obj.m_app["name"]))
                except FileExistsError as e:
                    pass
                
                shutil.copyfile(self.m_file, os.path.join(config["updates"]["folder"]["value"], "packages", site_conf_obj.m_app["name"], self.m_file.split(os.path.sep)[-1]))
            ...
            # FTP mode
                ...
                try:
                    ...
                    session.storbinary('STOR ' + config["updates"]["path"]["value"] + "/packages/" + site_conf_obj.m_app["name"] + "/" + self.m_file.split(os.path.sep)[-1], file)     # send the file
        ...
        elif self.m_action == "download_package" or self.m_action == "load_package_file":
            if self.m_action == "download_package":     
                ...
                config = utilities.util_read_parameters()
                if config["updates"]["source"]["value"] == "Folder":
                    shutil.copyfile(os.path.join(config["updates"]["folder"]["value"], "packages", site_conf_obj.m_app["name"], self.m_file), os.path.join("downloads", self.m_file))

                # FTP mode
                elif config["updates"]["source"]["value"] == "FTP":
                    try: 
                        session = ftplib.FTP(config["updates"]["address"]["value"], config["updates"]["user"]["value"], config["updates"]["password"]["value"])
                        session.retrbinary('RETR ' + config["updates"]["path"]["value"] +  '/packages/' + site_conf_obj.m_app["name"] + '/' + self.m_file, open(os.path.join("downloads", self.m_file), 'wb').write)     # send the file
                        session.quit()
                    except:
                ...
                if not ".zip" in self.m_file:
                    self.m_scheduler.emit_popup(scheduler.logLevel.error, "Package must be a .zip")
                    return
            ...
            self.m_scheduler.emit_status(self.get_name(), "Unpacking archive, this might take a while", 103)
            try:
                shutil.unpack_archive(os.path.join("downloads", self.m_file), os.path.join("ressources"), "zip")
                self.m_scheduler.emit_status(self.get_name(), "Unpacking archive, this might take a while", 100)
            except Exception as e:
                self.m_scheduler.emit_status(self.get_name(), "Unpacking archive, this might take a while", 101, str(e))       
        return 
...
def packager():
    ...
    if access_manager.auth_object.authorize_group("admin"):
        disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [3, 6, 3], subtitle="Package creation"))
        ...
        disp.add_display_item(displayer.DisplayerItemButton("pack", "Package creation"), 2)    

        to_upload_files = utilities.util_dir_structure(os.path.join("packages"), inclusion=[".zip"])
        disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [3, 6, 3], subtitle=""))
        disp.add_display_item(displayer.DisplayerItemText("Upload package"), 0)  
        disp.add_display_item(displayer.DisplayerItemInputFileExplorer("upload_package", None, [to_upload_files], ["Files to upload"], ["file-upload"], ["primary"], [False]), 1)
        disp.add_display_item(displayer.DisplayerItemButton("upload", "Package upload"), 2) 

        disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.VERTICAL, [3, 6, 3], subtitle="Package restoration"))
        ...
        if config["updates"]["source"]["value"] == "Folder":
            if not os.path.exists(os.path.join(config["updates"]["folder"]["value"])):
                ...
                content = utilities.util_dir_structure(os.path.join(config["updates"]["folder"]["value"], "packages", site_conf_obj.m_app["name"]), inclusion=[".zip"])
    ...
    return render_template('base_content_new.j2', content=disp.display(), target="packager.packager")
