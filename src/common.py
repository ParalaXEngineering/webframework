...
def download():
    ...
    if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
        path = os.path.join(os.path.dirname(sys.executable), "..", "downloads", request.args.to_dict()["file"])
    else:
...
def help():
    ...
    disp.add_display_item(displayer.DisplayerItemAlert(content, displayer.BSstyle.NONE), 0)

    return render_template('base_content_new.j2', content=disp.display(), target="")
...
def logs():
    ...
    for log_file in log_files:
        ...
        disp.add_master_layout(displayer.DisplayerLayout(displayer.Layouts.TABLE, ["Time", "Level", "File", "Function", "Line", "Message"], log_file[:-4].upper()))
        
        i = 0
        for log in reversed(log_entries):
            disp.add_display_item(displayer.DisplayerItemText(log["time"].strftime("%H:%M:%S")), 0, line=i)
            if log["level"] == "DEBUG":
                disp.add_display_item(displayer.DisplayerItemBadge(log["level"], displayer.BSstyle.PRIMARY), 1, line=i)
            elif log["level"] == "INFO":
                disp.add_display_item(displayer.DisplayerItemBadge(log["level"], displayer.BSstyle.INFO), 1, line=i)
            elif log["level"] == "WARNING":
                disp.add_display_item(displayer.DisplayerItemBadge(log["level"], displayer.BSstyle.WARNING), 1, line=i)
            else:
                disp.add_display_item(displayer.DisplayerItemBadge(log["level"], displayer.BSstyle.ERROR), 1, line=i)
            ...
            disp.add_display_item(displayer.DisplayerItemText(log["message"]), 5, line=i)
