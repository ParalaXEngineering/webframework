...
def _get_lockout_file_path() -> Path:
    ...
    On target (read-only filesystem), use /var/lib/dfnet/hmi/oufnis/ for persistence.
    ...
    if on_target:
        # On target: use persistent writable directory
        oufnis_dir = Path("/var/lib/dfnet/hmi/oufnis")
        oufnis_dir.mkdir(parents=True, exist_ok=True)
        return oufnis_dir / "failed_login.json"
    else:
