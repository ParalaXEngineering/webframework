...
# Cached placeholder image bytes (generated lazily on first missing-thumbnail request)
_placeholder_bytes: Optional[bytes] = None


def _get_thumbnail_placeholder() -> bytes:
    """Return cached placeholder JPEG bytes, generating on first call."""
    ...
    try:
        ...
        draw.rectangle([50, 35, 100, 115], outline=(150, 150, 150), width=2)
        draw.polygon([(85, 35), (100, 50), (85, 50)], outline=(150, 150, 150))
        for y in (65, 75, 85, 95):
            draw.line([(60, y), (90, y)], fill=(170, 170, 170), width=1)
        buf = BytesIO()
        img.save(buf, 'JPEG', quality=75)
        _placeholder_bytes = buf.getvalue()
    except Exception:
        # Minimal 1×1 grey JPEG (hardcoded, always valid)
        _placeholder_bytes = (
            b'\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x00\x00\x01\x00\x01\x00\x00'
            b'\xff\xdb\x00C\x00\x08\x06\x06\x07\x06\x05\x08\x07\x07\x07\t\t'
            b'\x08\n\x0c\x14\r\x0c\x0b\x0b\x0c\x19\x12\x13\x0f\x14\x1d\x1a'
            b'\x1f\x1e\x1d\x1a\x1c\x1c $.\'\",.#\x1c\x1c(7),\x01\x01\x01'
    ...
    try:
        ...
        if '.thumbs' in filepath:
            ...
            if not thumb_path.exists():
                logger.debug(f"Thumbnail missing, attempting regeneration: {filepath}")
                regenerated = file_manager.regenerate_thumbnail(filepath)
                if regenerated:
                    thumb_path = regenerated
                else:
                    logger.debug(f"Thumbnail unavailable, serving placeholder: {filepath}")
                    return send_file(BytesIO(_get_thumbnail_placeholder()), mimetype='image/jpeg')
